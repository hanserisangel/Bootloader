...
2. **远程 OTA 升级 + 差分升级**
适用于仅需要 OTA 升级 + 差分升级，并且删除本地升级和全量升级（以及串口交互菜单）。但是，需要注意的是，你的 APP 程序需要能够下载服务器的固件到外部 flash，要不然用不了这个部分的裁剪，关于这个 APP 程序的模板，请查看我的另一个存储库（还未更新）

3. **远程 OTA 升级 + 全量升级**
适用于仅需要 OTA 升级 + 全量升级，并且删除本地升级和差分升级（以及串口交互菜单）。但是，需要注意的是，你的 APP 程序需要能够下载服务器的固件到外部 flash，要不然用不了这个部分的裁剪，关于这个 APP 程序的模板，请查看我的另一个存储库（还未更新）
4. **本地固件升级 + 差分升级**
